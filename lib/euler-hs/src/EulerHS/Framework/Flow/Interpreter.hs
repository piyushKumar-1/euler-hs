...
-- TODO: remove after testing query time logger
-- alternatively just use seq or even deepseq
...
-- TODO: remove this after fixing slow order insert
...
interpretFlowMethod flowRt (L.RunDB conn sqlDbMethod next) = do
    ...
    let dbgLogger = R.runLogger T.RegularMode (R._loggerRuntime . R._coreRuntime $ flowRt)
                  ...
                  . show
    let dbgQueryExecTimeLogger = R.runLogger T.RegularMode (R._loggerRuntime . R._coreRuntime $ flowRt)
                               . L.logMessage' T.Debug ("RunDB took (ms):" :: T.Message)
    ...
    fmap (next . fst)  $ P.withRunMode runMode P.mkRunDBEntry $ case conn of
      ...
      _ -> do
        ...
        t2 <- getCPUTime
        let execTime = t2 - t1
        let timeStr = Text.pack $ printf ("%6.2fms"::String) ((fromIntegral execTime) / 1e9 :: Double)
        (!_lRes) <- dbgQueryExecTimeLogger timeStr
        pure (eRes, rawSql)
  where
