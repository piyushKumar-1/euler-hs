...
rExpire cName k t = do
  ...
  case res of
    Right r -> do
      -- L.logInfo @_ @Text "Redis expire" $ show r
      pure res
    Left err -> do
      L.logError @_ @Text "Redis expire" $ show err
      pure res
...
rDel cName ks = do
  ...
  case res of
    Right r -> do
      -- L.logInfo @_ @Text "Redis del" $ show r
      pure res
    Left err -> do
      L.logError @_ @Text "Redis del" $ show err
      pure res
...
rExists cName k = do
  ...
  case res of
    Right r -> do
      -- L.logInfo @_ @Text "Redis exists" $ show r
      pure res
    Left err -> do
      L.logError @_ @Text "Redis exists" $ show err
      pure res
...
rHget cName k f = do
  ...
  case res of
    ...
    Left err -> do
      L.logError @_ @Text "Redis hget" $ show err
      pure Nothing
...
rHset cName k f v = do
  ...
  case res of
    Right r -> do
      -- L.logInfo @_ @Text "Redis hset" $ show r
      pure res
    Left err -> do
      L.logError @_ @Text "Redis hset" $ show err
      pure res
...
rIncr cName k = do
  ...
  case res of
    Right r -> do
      -- L.logInfo @_ @Text "Redis incr" $ show r
      pure res
    Left err -> do
      L.logError @_ @Text "Redis incr" $ show err
      pure res
...
rSetT cName k v = do
  ...
  case res of
    Right r -> do
      -- L.logInfo @_ @Text "Redis set" $ show r
      pure res
    Left err -> do
      L.logError @_ @Text "Redis set" $ show err
      pure res
...
rGetT cName k = do
  ...
  -- L.logDebug @_ @Text "Raw value" $ show mv
  case mv of
    Right (Just val) -> do
      let v = A.eitherDecode $ BSL.fromStrict val
      case v of
        Left err -> do
          L.logError @_ @Text "Decoding error: " $ show err
          pure Nothing
        Right v' -> do
          -- L.logDebug @_ @Text "Decoded value" $ show v'
          pure $ Just v'
    Right (Nothing) -> pure Nothing
    Left err -> do
      L.logError @_ @Text "Redis get" $ show err
      pure Nothing
...
rSetex cName k v t = do
  ...
  case res of
    Right r -> do
      -- L.logInfo @_ @Text "Redis setex" $ show r
      pure res
    Left err -> do
      L.logError @_ @Text "Redis setex" $ show err
      pure res
